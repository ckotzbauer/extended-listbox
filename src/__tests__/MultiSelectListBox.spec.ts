...
describe("MultiSelectListBoxTest", () => {
    ...
    it("construct with searchbar watermark", (): void => {
        const { target } = TestHelper.generateMultipleList({ searchBar: true, searchBarWatermark: "Suche..." });
    ...
    it("explicit default value", (): void => {
        const { target, box } = TestHelper.generateMultipleList({}, ["A", "B", { id: "C", text: "C", selected: true }, "D"]);
    ...
    it("onValueChanged callback", (): void => {
        ...
        const { target } = TestHelper.generateMultipleList(options, ["A", "B", "C", "D"]);
